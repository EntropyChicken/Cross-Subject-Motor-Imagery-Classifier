...
The model is trained only on subjects 1--87. Subjects 88--109 are a
permanently held-out set and are never loaded by ``--train``.
"""
...
DEFAULT_MODEL_PATH = (
    ...
    / "models"
    / "csp_lda_imagery_s001_s087.joblib"
)


def _prepare_raw(
    edf_path: str | Path,
    expected_channels: Sequence[str] | None = None,
) -> mne.io.BaseRaw:
    ...
    if expected_channels is not None:
        missing = sorted(
            set(expected_channels).difference(raw.ch_names)
        )
        ...
        # CSP filters are channel-order dependent.
...
def _epochs_from_raw(
    ...
    if len(X) == 0:
        raise ValueError(
            "No complete 0.5--3.5 s cue-locked epochs could be created."
        )

    if X.shape[-1] < N_TIMES:
        raise ValueError(
            f"Expected at least {N_TIMES} samples per epoch after resampling; "
            f"got {X.shape[-1]}."
        )
...
def _load_subject_epochs(
    ...
    for path in paths:
        ...
        if expected_channels is not None:
            missing = sorted(
                set(expected_channels).difference(raw_part.ch_names)
            )

            if missing:
                raise ValueError(
                    f"Subject {subject_id} is missing expected EEG channels: "
                    f"{missing[:8]}"
                )
    ...
    # Concatenate runs before filtering, matching the notebook.
...
def train_model(
    ...
    """Fit CSP + LDA only on subjects 1--87 and save the model artifact."""
    ...
    # Same CSP + LDA pipeline used in the inter-subject notebook.
    pipeline = Pipeline(
        [
            (
                "CSP",
...
def predict_edf(
    ...
    """Return a left/right prediction and probabilities for each cue epoch.
    ...
    for event, predicted_class, probability in zip(
        epochs.events,
        predicted_classes,
        probabilities,
    ):
        ...
        predictions.append(
            {
                "cue_onset_seconds": round(
                    float(event[0] / raw.info["sfreq"]),
                    6,
                ),
                "predicted_label": LABELS[class_id],
                "probability_left_fist": float(
                    probability[class_to_column[0]]
                ),
                "probability_right_fist": float(
                    probability[class_to_column[1]]
                ),
            }
        )
...
def compare_with_edf_labels(
    ...
    """Diagnostic only: compare predictions with known T1/T2 labels."""
    ...
    for event, predicted_class, true_class in zip(
        epochs.events,
        predicted_classes,
        true_classes,
    ):
    ...
    return {
        ...
        "accuracy": float(
            np.mean(predicted_classes == true_classes)
        ),
        "n_trials": len(trials),
        "true_label_counts": {
            "Left_Fist": int(
                np.sum(true_classes == 0)
            ),
            "Right_Fist": int(
                np.sum(true_classes == 1)
            ),
        },
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "edf_path",
        nargs="?",
        help="Path to a raw EEGMMIDB EDF file",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        metavar="EDF_PATH",
        help="Diagnostic: compare predictions with known T1/T2 labels",
    )
...
def main() -> None:
    ...
    if args.train:
        ...
        if args.edf_path:
            raise SystemExit(
                "Provide either --train or an EDF path, not both."
            )

        artifact = train_model(args.model)

        print(
            f"Saved CSP + LDA model trained on "
            f"{len(artifact['training_subjects'])} subjects "
            f"to {args.model}. "
            f"Subjects "
            f"{artifact['held_out_subjects'][0]}--"
            f"{artifact['held_out_subjects'][-1]} "
            f"were not used."
        )
    ...
    if args.compare_labels:
        ...
        if args.edf_path:
            raise SystemExit(
                "Provide either an EDF path or --compare-labels paths, not both."
            )

        if not args.model.exists():
            raise SystemExit(
                f"Model artifact not found: {args.model}. "
                "Run `python prediction.py --train` first."
            )

        reports = [
            compare_with_edf_labels(
                edf_path,
                args.model,
            )
            for edf_path in args.compare_labels
        ]
    ...
    if not args.edf_path:
        raise SystemExit(
            "Provide an EDF path, or use --train to create the model artifact."
        )
